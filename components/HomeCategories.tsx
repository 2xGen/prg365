    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center mb-14">
          ...
          >
            Or browse by <span className="text-prg-red">category</span>
          </h2>
          <p className="mt-4 text-slate-600 text-lg leading-relaxed opacity-0 animate-fade-in-up" style={{ animationDelay: "0.1s", animationFillMode: "forwards" }}>
            Walking tours, castle visits, beer tours, day trips &amp; more. Each category shows top options to book.
          </p>
          <Link
            href="/best-tours-in-prague"
            className="mt-6 inline-flex items-center gap-2 text-prg-blue font-semibold hover:text-prg-blue-dark transition-colors opacity-0 animate-fade-in-up"
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
          {visible.map((p, i) => (
            <Link
              key={p.slug}
              href={`/${p.slug}`}
              className="group flex items-start gap-4 rounded-2xl border-2 border-slate-200 bg-white p-5 text-left transition-all duration-300 hover:border-prg-blue hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-prg-blue focus:ring-offset-2 opacity-0 animate-fade-in-up"
              style={{ animationDelay: `${0.15 + i * 0.04}s`, animationFillMode: "forwards" }}
            >
              <span className="flex-shrink-0 w-12 h-12 rounded-xl bg-prg-blue/10 text-prg-blue flex items-center justify-center group-hover:bg-prg-blue group-hover:text-white transition-colors">
